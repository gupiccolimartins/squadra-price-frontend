...
function Cidades() {
    ...
    const loadCities = async () => {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/api/cidades-estados/cidades?page=${currentPage}&size=${pageSize}`
        )
        if (!response.ok) {
  ...
  const filteredCities = useMemo(() => {
    ...
    return cities.filter((city) => city.cidade.toLowerCase().includes(normalized))
  }, [searchTerm, cities])
    ...
    <div className="locations-page">
      ...
      <main className="locations-container">
        <header className="locations-header">
          <h1>
            Cidades <span className="locations-divider">|</span>{' '}
            <Link className="locations-title-link" to="/Estados">
              Estados
            </Link>
          </h1>
        </header>

        <section className="locations-search" aria-label="Buscar cidades">
          <label htmlFor="locations-search-input">Busca:</label>
          <input
        ...
        <section className="locations-card">
          <table className="locations-table">
            ...
            <tbody>
              ...
              {!isLoading && !errorMessage && filteredCities.length === 0 && (
                <tr>
                  <td colSpan={4} className="locations-empty">
